...
One self-contained file at `<run>/report.html`, built from the template at `${CLAUDE_PLUGIN_ROOT}/templates/report.html` — **read that file fresh with the Read tool; never reconstruct it from memory** (the template is versioned and evolves — a remembered copy drifts from the current CSS and section set). **Keep the template's CSS untouched** (the look is plugin-owned and identical in every repo); fill the skeleton, delete unused optional sections, follow the rules in the template's head comment. The report is a *teaching document*, not a checklist: a reviewer should finish it able to retell how the feature works.

Sections (numbers = template order): sticky nav + hero (eyebrow, headline, lede) + **status pills** + **stat tiles** (tests/files/lines/slices — verdict readable in 5 seconds) + **reviewer triage block** (always present: the agent's one-sentence assessment — its own judgment, allowed here and nowhere else; "spend your review here" — 2–3 anchor links from the riskiest decision + the ○ items, each saying why human judgment matters there; "not touched" — the scary things the diff provably doesn't do, from the spec's out-of-scope + the diff, never invented reassurance) · **1 The story** (prose narrative: problem → shape of solution → data flow; no tables) · 2 Asked-vs-built (spec↔evidence; Met column = evidence chips) · 3 What changed (one card per slice with diffstat and ±/~ annotated rows) · **4 How it works** (2–4 real code excerpts ≤20 lines with the key lines marked, + diagrams harvested from the spec's Data & interfaces map, never invented: a data-model figure whenever the run touched schema — entity cards with the key fields marked and the join stated in plain words — and a flow figure shaped like the real flow: one strip when linear, two lanes joined by the shared artifact when a write is read back later, hand-drawn SVG only for genuine branch/fan-out; step text carries the run's real values so one concrete example is traced end-to-end) · **5 Rules & edge cases** (see below) · 6 Proof of work (terminal blocks with real output, **validation log**, persistence table, captioned screenshots) — §4, §5 and §6 each open with a one-line verdict (`.sectionlede`) so a heading-only scanner still gets the takeaway, and every figcaption states the CLAIM the figure is evidence for, standalone, never a description of the picture · **7 Try it yourself** (see below) · 8 Decisions & deviations (deviations as amber callouts, fixed-in-passing as green) · 9 Docs synced · 10 Noticed-but-not-touched (each item with a proof anchor) · 11 How to ship it (risk callout + what `/builder-ship` will do + paste-ready handoff block if the run produced text for someone else) · **12 To internalize** (3–5 reviewer questions, one-line answers in `<details>`).

**Evidence chips — claims wear how they were verified:** ✓ *live* (this run observed it: command ran, test passed, UI driven) · ◐ *fixture/inference* (test data, code reading, derived) · ○ *needs a human*. Used in the §2 Met column (a partially-met AC is ◐ + a note — never "✓ with an asterisk"), the §5 rule/edge rows, the §6 validation log (every load-bearing claim beyond gate+e2e: claim · how verified · chip), and §10 items. Never award ✓ to something the run didn't actually observe. **Every ○ also carries a ship-gate tag** — `decide before ship` / `ship, then watch` / `fine as-is` — and every "decide before ship" item appears in the hero triage links: ○ says *who* must act, the gate says *when*.

**§5 Rules & edge cases (always present):** the behavioral contract — the section that transfers **ownership** from the agent to the human. The reader must leave able to answer *"what does the system do when X?"* without reading the code. Two tables, **harvested, never invented here**: **the rules** — every business rule this run implemented or newly relies on, in plain English, with where it lives and the test that pins it (sources: §4's marked code lines, the spec); **at the edges** — the boundary/degenerate inputs and the system's **observed** behavior (what a test or live run actually showed — never the intended behavior; the chip says which), sourced from the spec's "Edge cases & behavior" list, the ALIGN grill, and the tests. An edge nobody pinned gets ○ here *and* a row in §10 — an unpinned edge is a decision the human hasn't made yet; surfacing it is this section's job, not a failure.

**§7 Try it yourself (always present):** teach the reviewer the feature **by doing, without this session** — from a clean checkout of the branch, assuming none of the session's running state. Prereqs first (services, env, seeds — exact commands or a doc pointer), then 3–6 numbered steps: intent sentence, copy-paste command block, and a "what you'll see" box (the exact success signal + likeliest failure and fix). Steps come in **three kinds, include all three** (each labeled): **reproduce** — re-run the proof, with at least one step exercising the feature the way a *user* would (UI click-path or API call), not just the tests; **inspect** — verify the *data* first-hand (query the persisted rows / read the raw payload via psql/duckdb/curl, not through the app — seeing the row beats being shown it); **explore** — make a *business rule* visible by varying one input: a "what do you expect?" predict box *before* the command, then what actually happens, why, and where the rule lives — **pick the rule/edge from §5's tables** (the rule with the most surprising edge row is the best candidate); discovering a rule beats reading it. A step that can't be reproduced outside the session (prod creds, teammate's data) says so honestly with a ○ chip — never pretend. §12's questions tie back: at least one targets the explore step's rule, and answers the reader saw first-hand cite their §7 step.

Hard rules: terminal blocks show **real output, never fabricated**, each with a context label in its title bar and a "what it means" line after; screenshots resized to ≤1200px and re-encoded JPEG ~70 before embedding (<150 KB each); code excerpts are real code; the footer records the HEAD commit the report reflects. When done: **`git add` the report** (staged so `/builder-ship` step 2 commits it — an untracked report gets orphaned; pilot 3), **show the report's path**, and tick the ledger's REPORT row with the path and set the plan's Status line to DONE (small path: no ledger/plan — skip, the report is the record).
