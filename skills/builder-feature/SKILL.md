...
Sections (numbers = template order): sticky nav + hero (eyebrow, headline, lede) + **status pills** + **stat tiles** (tests/files/lines/slices — verdict readable in 5 seconds) · **1 The story** (prose narrative: problem → shape of solution → data flow; no tables) · 2 Asked-vs-built (spec↔evidence; Met column = evidence chips) · 3 What changed (one card per slice with diffstat and ±/~ annotated rows) · **4 How it works** (2–4 real code excerpts ≤20 lines with the key lines marked, + the CSS pipeline strip of the actual user action with real names — hand-drawn SVG only for genuinely non-linear flows) · 5 Proof of work (terminal blocks with real output, **validation log**, persistence table, captioned screenshots) · **6 Try it yourself** (see below) · 7 Decisions & deviations (deviations as amber callouts, fixed-in-passing as green) · 8 Docs synced · 9 Noticed-but-not-touched (each item with a proof anchor) · 10 How to ship it (risk callout + what `/builder-ship` will do + paste-ready handoff block if the run produced text for someone else) · **11 To internalize** (3–5 reviewer questions, one-line answers in `<details>`).

**Evidence chips — claims wear how they were verified:** ✓ *live* (this run observed it: command ran, test passed, UI driven) · ◐ *fixture/inference* (test data, code reading, derived) · ○ *needs a human*. Used in the §2 Met column (a partially-met AC is ◐ + a note — never "✓ with an asterisk"), the §5 validation log (every load-bearing claim beyond gate+e2e: claim · how verified · chip), and §9 items. Never award ✓ to something the run didn't actually observe.

**§6 Try it yourself (always present):** teach the reviewer the feature **by doing, without this session** — from a clean checkout of the branch, assuming none of the session's running state. Prereqs first (services, env, seeds — exact commands or a doc pointer), then 3–6 numbered steps: intent sentence, copy-paste command block, and a "what you'll see" box (the exact success signal + likeliest failure and fix). Steps come in **three kinds, include all three** (each labeled): **reproduce** — re-run the proof, with at least one step exercising the feature the way a *user* would (UI click-path or API call), not just the tests; **inspect** — verify the *data* first-hand (query the persisted rows / read the raw payload via psql/duckdb/curl, not through the app — seeing the row beats being shown it); **explore** — make a *business rule* visible by varying one input: a "what do you expect?" predict box *before* the command, then what actually happens, why, and where the rule lives — prefer the variation where the rule bites (the cap, the filter, the fallback); discovering a rule beats reading it. A step that can't be reproduced outside the session (prod creds, teammate's data) says so honestly with a ○ chip — never pretend. §11's questions tie back: at least one targets the explore step's rule, and answers the reader saw first-hand cite their §6 step.
