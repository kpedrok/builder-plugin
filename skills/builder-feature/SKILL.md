...
**UI-facing feature?** If the change adds or reshapes a visual surface and the direction isn't already settled (mockup in the ticket, an existing pattern to copy), route through the `builder-prototype` skill (invoke it by name — `/builder-prototype` / `$builder-prototype`) before PLAN: 5 genuinely distinct variations in one disposable self-contained HTML file, the human picks (or blends), then plan only the winner. Reuse the design system per `.harness/map/design.md` — never invent components that already exist. Trivial UI (a field in an existing form) skips this — say so in one line.
