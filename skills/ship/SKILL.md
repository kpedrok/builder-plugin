...
5. **Close the plan.** Move `.harness/plans/<feature>.md` to `.harness/plans/archive/` (its Status is DONE from REPORT) so resume scans stay clean — and commit the move; the ship must end with a clean tree.
