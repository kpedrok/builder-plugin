...
**§6 Try it yourself (always present):** teach the reviewer to reproduce the proof **without this session** — from a clean checkout of the branch, assuming none of the session's running state. Prereqs first (services, env, seeds — exact commands or a doc pointer), then 3–6 numbered steps: intent sentence, copy-paste command block, and a "what you'll see" box (the exact success signal + likeliest failure and fix). The last step exercises the feature the way a *user* would (UI click-path or API call), not just the tests. A step that can't be reproduced outside the session (prod creds, teammate's data) says so honestly with a ○ chip — never pretend.
