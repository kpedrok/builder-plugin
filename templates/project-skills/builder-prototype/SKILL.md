...
Explore a UI idea as a few genuinely distinct variations on a throwaway route/page before building it for real. Pick a winner (or a blend) with the human, then hand off to the `builder-feature` pipeline to build only that.
...
- The chosen direction goes through `builder-feature` (ALIGN → PLAN → …) like any other build.
