...
5. **Update the tracker.** Move the ticket to its post-merge/ready state and comment with the PR link(s) — one comment covering all PRs (per `.harness/agents/tracker.md`). Skip if that mapping records no tracker.
